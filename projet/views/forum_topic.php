...
if(isset($_GET['categorie']) AND !empty($_GET['categorie'])) {
    ...
    if(@$id_categorie) {
        ...
        $req = "SELECT *, f_topics.id AS topicid FROM f_topics 
            ...
            <table class="forum" border="10" align="center">
                ...
                <tr>
                    <td class="main_topic">
                        ...
                        <a href="/projet/views/topic.php?id=<?= $t['f_topics.id'] ?>"><?php echo $t['sujet'] ?></a><br /> 
                    ...
                    <td class="sub-info">
                        <?php $rep = $bdd->prepare('SELECT f_messages.* FROM f_messages 
                        LEFT JOIN f_topics ON f_topics.id = f_messages.id_topic 
                        WHERE f_topics.id = ? 
                        ORDER BY f_messages.dateheure_post ');
                        $rep->execute(array($t['topicid']));
                        if($rep->rowCount() > 0) {
                            $rep = $rep->fetch();
