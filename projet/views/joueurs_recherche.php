...
if(isset($_GET['idjoueur']) AND !empty($_GET['idjoueur']) ) {
    ...
    if(isset($_POST['joueur_com_submit'],$_POST['joueur_com'])) {
        ...
        if(isset($_SESSION['id'])) {
            if(!empty($commentaire)) {
                $ins = $bdd->prepare('INSERT INTO espace_com_joueur (id_joueur,id_posteur,contenu,dateheure_post) VALUES (?,?,?,NOW())');
                $ins->execute(array($get_idjoueur,$_SESSION['id'],$commentaire));
    ...
    $reponses = $bdd->prepare('SELECT * FROM espace_com_joueur WHERE id_joueur = ? LIMIT '.$depart.','.$reponsesParPage);
        ...
        <div id="bloc_page">
           ...
           <div id="banniere_image">
            ...
            <table class="reponse" border=3>
                ...
                <tr>
                    <td style="color:white; text-align:center" ><?php $pseudo = $bdd->prepare('SELECT * FROM membre LEFT JOIN espace_com_tactique ON espace_com_tactique.id_posteur=membre.id WHERE espace_com_tactique.id_posteur = ?');
